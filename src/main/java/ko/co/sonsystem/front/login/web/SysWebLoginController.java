package ko.co.sonsystem.front.login.web;
...
import ko.co.sonsystem.front.login.service.SysWebLoginService;
...
@Slf4j
@RestController
@Tag(name="SysWebLoginController",description = "로그인 관련")
public class SysWebLoginController {

	/** SysLoginService */
	@Resource(name = "sysWebLoginService")
	private SysWebLoginService loginService;
	...
	@PostMapping(value = "/sys/web/auth/login", consumes = {MediaType.APPLICATION_JSON_VALUE , MediaType.TEXT_HTML_VALUE})
	...
	@PostMapping(value = "/sys/web/auth/login-jwt")
	...
	@GetMapping(value = "/sys/web/auth/logout")
